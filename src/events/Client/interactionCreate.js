        ...
        if (interaction.isCommand() || interaction.isContextMenu()) {
            ...
            if (SlashCommands.owner && interaction.author.id !== `${client.owner}`) {
                await interaction.reply({
                    content: `Only <@836958855866089512> can use this command!`, ephemeral: true
                }).catch(() => { });
            }
            ...
            if (!interaction.member.permissions.has(SlashCommands.userPrams || [])) {
                return await interaction.reply({ content: `I Need Permission to Work this \`${SlashCommands.userPrams.join(", ")}\``, ephemeral: true })

            }
            if (!interaction.guild.me.permissions.has(SlashCommands.botPrams || [])) {
                return await interaction.reply({ content: `You Need this \`${SlashCommands.botPrams.join(", ")}\` Permission to Work this command!`, ephemeral: true })
