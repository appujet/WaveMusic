...
class SetupButtons extends Event {
    ...
    async run(interaction) {
        ...
        if (interaction.guild.members.cache.get(this.client.user.id).voice.channel &&
            interaction.guild.members.cache.get(this.client.user.id).voice.channelId !==
                interaction.member.voice.channelId)
            return await buttonReply(interaction, `You are not connected to ${interaction.guild.me.voice.channel} to use this buttons.`, this.client.color.red);
        const player = this.client.queue.get(interaction.guildId);
        if (!player)
            return await buttonReply(interaction, `There is no music playing in this server.`, this.client.color.red);
        if (!player.queue)
            return await buttonReply(interaction, `There is no music playing in this server.`, this.client.color.red);
        if (!player.current)
            return await buttonReply(interaction, `There is no music playing in this server.`, this.client.color.red);
        ...
        const embed = this.client
            ...
            .setDescription(`[${title}](${uri}) - ${player.current.info.isStream ? 'LIVE' : this.client.utils.formatTime(length)} - Requested by ${player.current.info.requester}`)
            .setImage(icon);
        if (message) {
            switch (interaction.customId) {
                case 'LOW_VOL_BUT': {
                    ...
                    await buttonReply(interaction, `Volume set to ${vol.toFixed()}%`, this.client.color.main);
                    await message.edit({
                        embeds: [
                            embed.setFooter({
                ...
                case 'HIGH_VOL_BUT': {
                    ...
                    await buttonReply(interaction, `Volume set to ${vol2.toFixed()}%`, this.client.color.main);
                    await message.edit({
                        embeds: [
                            embed.setFooter({
                ...
                case 'SHUFFLE_BUT':
                    ...
                    await buttonReply(interaction, `Shuffle set to ${player.shuffle ? `on` : `off`}.`, this.client.color.main);
                    await message.edit({
                        embeds: [
                            embed.setFooter({
                ...
                case 'REWIND_BUT': {
                    ...
                    if (time < 0)
                        return await buttonReply(interaction, `You cannot rewind the music more than the length of the song.`, this.client.color.main);
                    player.seek(time);
                ...
                case 'FORWARD_BUT': {
                    ...
                    if (time2 > player.current.info.length)
                        return await buttonReply(interaction, `You cannot forward the music more than the length of the song.`, this.client.color.main);
                    player.seek(time2);
