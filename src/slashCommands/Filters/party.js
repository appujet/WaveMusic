...
module.exports = {
  ...
  run: async (client, interaction, prefix) => {
    ...
    const but2 = new MessageButton().setCustomId('Party_but').setLabel('ON').setStyle('PRIMARY');
    ...
    collector.on('collect', async (b) => {
      ...
      if (b.customId === 'clear_but') {
        ...
        return await b.editReply({
          embeds: [embed1.setDescription(`${emojiequalizer} Party Mode Is \`OFF\``)],
          components: [row2],
        });
        ...
        return await b.editReply({
          embeds: [embed1.setDescription(`${emojiequalizer} Party Mode Is \`ON\``)],
          components: [row1],
        });
      }
    });
