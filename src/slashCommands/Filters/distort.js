...
module.exports = {
    name: "distort",
    description: "Sets Distort Filter.",
    owner: false,
    ...
    collector.on("end", async () => {
      if (!m) return;
      await m.edit({
        embeds: [embed1.setDescription(`Time is Out type again ${prefix}distort`)],
        components: [
    ...
    collector.on("collect", async (b) => {
      ...
      if (b.customId === "clear_but") {
        ...
        return await b.editReply({
          embeds: [
            embed1.setDescription(`${emojiequalizer} Distort Mode Is \`OFF\``),
          ],
          components: [row2],
        });
        ...
        return await b.editReply({
          embeds: [
            embed1.setDescription(`${emojiequalizer} Distort Mode Is \`ON\``),
          ],
          components: [row1],
        });
      }
