...
module.exports = {
  ...
  run: async (client, interaction, prefix) => {
    ...
    const but2 = new MessageButton().setCustomId('Speed_but').setLabel('ON').setStyle('PRIMARY');
    ...
    collector.on('end', async () => {
      if (!m) return;
      await m.edit({
        embeds: [embed1.setDescription(`Time is Out type again ${prefix}speed`)],
        components: [
    ...
    collector.on('collect', async (b) => {
      ...
      if (b.customId === 'clear_but') {
        ...
        return await b.editReply({
          embeds: [embed1.setDescription(`${emojiequalizer} Speed Mode Is \`OFF\``)],
          components: [row2],
        });
       ...
       await player.player.setFilters({
        ...
        return await b.editReply({
          embeds: [embed1.setDescription(`${emojiequalizer} Speed Mode Is \`ON\``)],
          components: [row1],
        });
      }
    });
