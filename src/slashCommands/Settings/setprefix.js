...
module.exports = {
  ...
  run: async (client, interaction, prefix) => {
    ...
    if (!pre[0]) {
      ...
      return await interaction.editReply({ embeds: [embed] }).then(msg => { setTimeout(() => { msg.delete() }, 5000) }).catch(() => { });
    }
    if (pre[1]) {
      const embed = new MessageEmbed()
    ...
    if (data) {
      ...
      const update = new MessageEmbed()
        ...
        .setColor(client.embedColor)
      return await interaction.editReply({ embeds: [update] }).then(msg => { setTimeout(() => { msg.delete() }, 5000) }).catch(() => { });
    } else {
