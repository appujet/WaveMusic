...
module.exports = {
    ...
    if(!player) return await interaction.editReply({
        ...
        if(!player.queue.size || player.queue.size === 0) {
            ...
            const embed = new MessageEmbed().setColor(client.embedColor).setDescription(`<a:playing:919580939720482816>  **Now playing** [${player.queue.current.title}](${player.queue.current.uri}) • \`[ ${pms(player.position)} / ${pms(player.queue.current.duration)} ]\` ➥ [${player.queue.current.requester}]`);
            ...
            if(player.queue.size < 10 || player.queue.totalSize < 10) {
                ...
                const embed2 = new MessageEmbed().setTitle(`${interaction.guild.name} Server Queue`).setColor(client.embedColor).setDescription(`<a:playing:919580939720482816> **Now playing**\n[${player.queue.current.title}](${player.queue.current.uri}) • \`[ ${pms(player.position)} / ${pms(player.queue.current.duration)} ]\` ➥ [${player.queue.current.requester}]\n\n<:queue:931873502422761492> **Queued Songs**\n${pages[page]}`).setFooter({name:`Page ${page + 1}/${pages.length}`, iconURL: interaction.user.displayAvatarURL({ dynamic: true })})

                await interaction.editReply({
                    embeds: [embed2]
                }).catch(() => {});
            } else {
                const embed3 = new MessageEmbed().setTitle(`${interaction.guild.name} Server Queue`).setColor(client.embedColor).setDescription(`<a:playing:919580939720482816> **Now playing**\n[${player.queue.current.title}](${player.queue.current.uri}) • \`[ ${pms(player.position)} / ${pms(player.queue.current.duration)} ]\` ➥ [${player.queue.current.requester}]\n\n<:queue:931873502422761492> **Queued Songs**\n${pages[page]}`).setFooter({name:`Page ${page + 1}/${pages.length}`, iconURL: interaction.user.displayAvatarURL({ dynamic: true })})
                ...
                collector.on("collect", async (button) => {
                    if(button.customId === "queue_cmd_but_1_app") {
                        ...
                        const embed4 = new MessageEmbed().setColor(client.embedColor).setDescription(`<a:playing:919580939720482816> **Now playing**\n[${player.queue.current.title}](${player.queue.current.uri}) • \`[ ${pms(player.position)} / ${pms(player.queue.current.duration)} ]\` ➥ [${player.queue.current.requester}]\n\n<:queue:931873502422761492> **Queued Songs**\n${pages[page]}`).setFooter({text: `Page ${page + 1}/${pages.length}`, iconURL: button.user.displayAvatarURL({ dynamic: true })})
                        ...
                        const embed5 = new MessageEmbed().setColor(client.embedColor).setDescription(`<a:playing:919580939720482816> **Now playing**\n[${player.queue.current.title}](${player.queue.current.uri}) • \`[ ${pms(player.position)} / ${pms(player.queue.current.duration)} ]\` ➥ [${player.queue.current.requester}]\n\n<:queue:931873502422761492> **Queued Songs**\n${pages[page]}`).setFooter({Text: `Page ${page + 1}/${pages.length}`, iconURL: button.user.displayAvatarURL({ dynamic: true })})
