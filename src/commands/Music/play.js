...
module.exports = {
  ...
  execute: async (message, args, client, prefix) => {
    ...
    return message.reply(result.type === "PLAYLIST" ? { embeds: [new MessageEmbed().setColor(client.embedColor).setDescription(`Queued ${tracks.length} from ${result.playlistName}` )]} : { embeds: [new MessageEmbed().setColor(client.embedColor).setDescription(`Queued [${tracks[0].title}](${tracks[0].uri})`)]})
  }
};
