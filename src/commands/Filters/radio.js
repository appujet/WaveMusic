...
module.exports = {
  ...
  execute: async (message, args, client, prefix) => {
    ...
    collector.on("end", async () => {
      ...
      await m.edit({
        embeds: [
          embed1.setDescription(`Time is Out type again ${prefix}radio`),
        ],
        components: [
    ...
    collector.on("collect", async (b) => {
      ...
      if (b.customId === "clear_but") {
        ...
        return await b.editReply({
          embeds: [
            embed1.setDescription(
              `${emojiequalizer} Radio Mode Is \`OFF\``
            ),
          ],
          components: [row2],
        });
        ...
        return await b.editReply({
          embeds: [
            embed1.setDescription(`${emojiequalizer} Radio Mode Is \`ON\``),
          ],
          components: [row1],
        });
      }
